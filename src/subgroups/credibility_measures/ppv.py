...
"""This file contains the implementation of the Positive Predictive Value (PPV) credibility measure.
"""
...
class PPV(CredibilityMeasure):
    """This class defines the Positive Predictive Value (PPV) credibility measure.
    """
    
    _singleton = None
    __slots__ = ("_threshold",)
    
    def __new__(cls, threshold: float = None) -> 'PPV':
        if PPV._singleton is None:
            PPV._singleton = object().__new__(cls)
        return PPV._singleton

    def __init__(self, threshold: float = None) -> None:
        """Constructor of the class PPV.
    ...
    def __call__(self, dict_of_parameters : dict[str, Union[int, float]]) -> bool:
        """Compute the PPV quality measure and checks if it meets the threshold.
        ...
        :return: wether the computed value for the PPV quality measure meets the threshold or not.
        """
        if self._threshold is None:
            raise ValueError("The threshold for the PPV credibility measure is not set.")
